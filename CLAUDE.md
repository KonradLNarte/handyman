...
1. **7 tables only** — tenants, events, nodes, edges, grants, blobs, dicts
...
5. **RLS on every table** — federation via grants table + cross-tenant edges, never bypass
...
pnpm db:seed      # Seed metatype bootstrap + platform defaults
```
...
1. **data-model** — table schemas, RLS, Drizzle, Zod, type nodes, grants
...
See `docs/specs/gen2/index.md` for the current (gen2) system specification.
See `docs/specs/resonansia-mcp-server-gen1-spec.md` for the gen1 spec archive.
